...
book = openpyxl.load_workbook("Not in NSE DB.xlsx")
scripts = book.get_sheet_by_name("Sheet")
...
for i in range(1,scripts.max_row+1):
    script_id = scripts['A'+str(i)].value
    ...
    try:
        ...
        if br.title()=="Symbol lookup from Yahoo Finance":
            not_in_yahoo.append(script_id)
            continue
    except:
        not_in_yahoo.append(script_id)
        continue
    '''
    if script_id not in DB_list:
