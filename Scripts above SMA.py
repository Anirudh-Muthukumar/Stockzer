...
print "Computing stocks breaking moving average.......\n"
...
try:
    os.remove("Scripts above SMA.xlsx")
except:
    True
wb_high.save("Scripts above SMA.xlsx")

print "\nSuccessfully computed stocks above SMA!\n\n"   
...
sys.argv = [str(datetime.now().date()), "Scripts above SMA", str(start.time()), str(end.time()), int(hours), int(minutes), int(seconds)]
