...
ws_low['A1'], ws_low['B1'] = "S.No", "Script"
row_low = 2
...
for i in range(2,scripts.max_row+1):                    #scripts.max_row+1
    ...
    if abs(cmp-low_52)<=tolerance or cmp<low_52:
        ws_low['A'+str(row_low)] = row_low-1
        ws_low['B'+str(row_low)] = bs['A'+str(i)].value                
    if abs(cmp-high_52)<=tolerance or cmp>high_52:
...
try:
    os.remove("Stocks in 52 week low.xlsx") 
    os.remove("Stocks in 52 week high.xlsx") 
except:
    True
