...
ws_high.title = "Stocks to be sold off"
...
try :
    os.remove("Scripts below EMA.xlsx")    
except:
    True

wb_ema.save("Scripts below EMA.xlsx")

print "\nSuccessfully computed stocks below EMA!\n\n"   
...
sys.argv = [str(datetime.now().date()), "Scripts below EMA", str(start.time()), str(end.time()), int(hours), int(minutes), int(seconds)]
